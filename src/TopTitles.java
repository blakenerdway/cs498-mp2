...
public class TopTitles extends Configured implements Tool {
    ...
    public static class TitleCountMap extends Mapper<Object, Text, Text, IntWritable> {
        ...
        @Override
        // Will return a list of maps:   List-{Map1-{<word1, count1>, <word2, count2>, etc... }, Map2-{word1,count1>, <word2, count2>, etc}, etc}
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
            //TODO
            String line = value.toString();
            String[] words = line.split(this.delimiters);
            for (String word : words){
                String cleanedWord = word.toLowerCase().trim();
                if (this.stopWords.contains(cleanedWord)){
                    continue;
                }
                Text mapKey = new Text(cleanedWord);
    ...
    public static class TopTitlesMap extends Mapper<Text, Text, NullWritable, TextArrayWritable> {
        private Map<Integer, Text> titleToCountMap = new TreeMap<>();
        ...
        protected void cleanup(Context context) throws IOException, InterruptedException {
            //TODO
            String[] strValues = new String[titleToCountMap.values().size()];
            for (int i = 0; i < titleToCountMap.size(); i++) {
                Text text = titleToCountMap.get(i);
                strValues[i] = text.toString();
            }
            context.write(NullWritable.get(), new TextArrayWritable(strValues));
        }
    }

    /**
     * Find the top x words
     */
    public static class TopTitlesReduce extends Reducer<NullWritable, TextArrayWritable, Text, IntWritable> {
        // TODO
        private TreeMap<Integer, Text> titleToCountMap = new TreeMap<>();
